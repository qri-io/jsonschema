...
func (m MultipleOf) ValidateFromContext(schCtx *SchemaContext, errs *[]KeyError) {
	if num, ok := schCtx.Instance.(float64); ok {
		...
		if float64(int(div)) != div {
			AddError(errs, schCtx.Local.DocPath, schCtx.Instance, fmt.Sprintf("must be a multiple of %f", m))
		}
	}
}
...
func (m Maximum) ValidateFromContext(schCtx *SchemaContext, errs *[]KeyError) {
	if num, ok := schCtx.Instance.(float64); ok {
		if num > float64(m) {
			AddError(errs, schCtx.Local.DocPath, schCtx.Instance, fmt.Sprintf("must be less than or equal to %f", m))
		}
	}
}
...
func (m ExclusiveMaximum) ValidateFromContext(schCtx *SchemaContext, errs *[]KeyError) {
	if num, ok := schCtx.Instance.(float64); ok {
		if num >= float64(m) {
			AddError(errs, schCtx.Local.DocPath, schCtx.Instance, fmt.Sprintf("%f must be less than %f", num, m))
		}
	}
}
