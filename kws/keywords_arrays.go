...
// Validate implements the Validator interface for Items
func (it Items) ValidateFromContext(schCtx *SchemaContext, errs *[]ValError) {
	jp, err := jsonpointer.Parse(schCtx.Local.DocPath)
	if err != nil {
		AddError(errs, schCtx.Local.DocPath, nil, fmt.Sprintf("invalid property path: %s", err.Error()))
	}
	...
	if arr, ok := data.([]interface{}); ok {
		if it.single {
			...
			for i, vs := range it.Schemas {
				if i < len(arr) {
					d, _ := jp.Descendant(strconv.Itoa(i))
					vs.ValidateFromContext(d.String(), arr[i], errs)
				}
			}
		}
	}
